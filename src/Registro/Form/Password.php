...
class Registro_Form_Password extends Gatuf_Form {
	public function initFields($extra=array()) {
		$this->fields['account'] = new Gatuf_Form_Field_Varchar(
		                               array('required' => true,
		                                     'label' => 'Tu nick o correo',
		                                     'help_text' => 'Ingresa tu nick o correo para recuperar la contraseña',
		));
	}
	...
	public function clean_account () {
		...
		$users = Gatuf::factory ('Registro_User')->getList(array ('filter' => $sql->gen()));
	...
	function save ($commit=true) {
		...
		foreach ($users as $user) {
			if ($user->active) {
				$return_url = Gatuf_HTTP_URL_urlForView ('Registro_Views::passwordRecoveryInputCode');
				$tmpl = new Gatuf_Template('registro/user/recuperarcontra-email.txt');
				...
				$url = Gatuf::config ('url_base').Gatuf_HTTP_URL_urlForView ('Registro_Views::passwordRecovery', array ($code), array (), false);
				$urlic = Gatuf::config ('url_base').Gatuf_HTTP_URL_urlForView ('Registro_Views::passwordRecoveryInputCode', array (), array (), false);
				...
				$email = new Gatuf_Mail (Gatuf::config ('from_email'), $user->email, 'Recuperar contraseña - Cursos de invierno');
